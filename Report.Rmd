...
```
```{r, echo=FALSE}
knitr::kable(head(edx), caption = "EDX Header After Formating")
```
...
- __Users tened to rate in whole numbers insted of fractions.__
...
edx%>%
  ...
  ylab(label = "Avrage Rating Given By User")+
...
edx %>%
  ...
  xlab(label = "Avrage Rating Given By User" ) +
...
- Users tends to give ratings above the center of used scale **2.5**, at an average of **3.5**.
